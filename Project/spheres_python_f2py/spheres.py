...
import fortran_utils.module_spheres_fortran as fr


def spheres(Rvt=None, n =3 , Ncolissions = 100,r= None,L=1.0,kT=1,speedScale = 1.0 ,Iterations=1):
    ...
    c_time = init_collision_time(pos, vel, pos_trans[0], sigma)
    ...
    pos_f = np.asfortranarray(np.array(pos))
    vel_f = np.asfortranarray(np.array(vel))
    c_time_f = np.asfortranarray(np.array(c_time))
    sigma_f = sigma
    ...
    for _ in range(Iterations):
        for nc in range(Ncollisions):
            ...
            t_stop = tm.time()
            timing[0] += t_stop - t_start

            t_start = tm.time()
            tcol_f, icol_f, jcol_f = fr.retrieve_collision_info(c_time_f)
            icol_f -= 1
            jcol_f -= 1
            t_stop = tm.time()
            timing[1] += t_stop - t_start
            ...
            # fr.write_properties(nc, mom, kin, tcol, icol, jcol, delta_vel_f)
            # io.write_properties(nc, mom, kin, tcol_f, icol_f, jcol_f, delta_vel)

            t_start = tm.time()
            pos, vel, delta_vel = advance_simulation(pos, vel, tcol, icol, jcol, sigma_sq, sigma, Ns, nc,pos_trans[0], conf, L)
            t_stop = tm.time()
            timing[2] += t_stop - t_start

            # pos_f = np.asfortranarray(pos)
            # vel_f = np.asfortranarray(vel)
            t_start = tm.time()
            delta_vel_f = fr.advance_simulation(tcol_f, icol_f, jcol_f, sigma_f, pos_f, vel_f)
            t_stop = tm.time()
            timing[3] += t_stop - t_start
            # pos = np.ascontiguousarray(pos_f)
            # vel = np.ascontiguousarray(vel_f)
            # delta_vel = np.ascontiguousarray(delta_vel_f)


            t_start = tm.time()
            mom, kin = compute_properties(vel)
            t_stop = tm.time()
            timing[4] += t_stop - t_start

            t_start = tm.time()
            mom_f, kin_f = fr.compute_properties(vel_f)
            t_stop = tm.time()
            timing[5] += t_stop - t_start
            ...
            t_stop = tm.time()
            timing[6] += t_stop - t_start


            c_time_f = np.asfortranarray(c_time)
            pos_f = np.asfortranarray(pos)
            vel_f = np.asfortranarray(vel)

            t_start = tm.time()
            fr.update_collisions_table(pos_f, vel_f, sigma_f, tcol_f, icol_f, jcol_f, c_time_f)
            t_stop = tm.time()
            timing[7] += t_stop - t_start
            ...
            t_start = tm.time()
            fr.write_properties(nc, mom_f, kin_f, tcol_f, icol_f, jcol_f, delta_vel_f)
            t_stop = tm.time()
            timing[0] += t_stop - t_start
            ...
            for i, u in enumerate(val1):
                for j, v1 in enumerate(u):
                    test.icheck(v1 and val2[i, j]
                                ...
                                , False)
    # t_stop = tm.time()
    print Iterations, "Iterations running time:", str(timing), " [seconds]"
...
def init_collision_time(pos, vel, pos_trans, sigma, L=1.0):
    Ns = np.shape(pos)[0]
    c_time = conf.INF * np.ones((Ns, Ns))

    for mol in range(Ns):
        time = update_ctime_row(mol, pos, vel, sigma, pos_trans)
        c_time[mol, :] = time
        c_time[:, mol] = time
    test.icheck(np.any(c_time) < 0, "initial collision time with negtive t_cols !!!", True)
    return c_time