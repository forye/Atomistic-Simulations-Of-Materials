...
# import subrutines_numpy as sb


def init_parameters(n=0,Ncollisions = 1000 , r=0 , Rvol = 0 , L=1.0):
    Rv= conf.VOLUME_RATIO #1.5 #default Rv
    ...
    Ns=(n**3)*4
    if r: Rv = (math.sqrt(2.0)/Ns) *((L/float(r))**3.)#imposed radius(diameter??! is r sigma?
    if Rvol: Rv = Rvol # given (imposed) v/vo
    # nspheres,rvolume,ncollisions = read_input()
    test.icheck(not validate_input(Ns,Rv) , ' bad input Ns= '+ str(Ns) + " Rv= " + str(Rv))
    L=float(L)#float
    sigma = compute_diameter(Ns,Rv) *L
    Ncollisions = int(Ncollisions)
    #init positions
...
def validate_input(nspheres, rvolume):
    ...
    # ! Make sure the reduced volume makes some sense:
    if (rvolume < 1.0):
...
def init_velocities(Ns,kT,speedScale):
    mass = conf.MASS
    speed =speedScale * math.sqrt(3.)## here Ive change ( reduced by 3 for 32 spheres kt=1
    u = np.random.random(Ns)# random.random()
    v = np.random.random(Ns)#random.random()
    theta = 2.*np.pi * u
    phi =  np.arccos(2.* v - 1  )# phi = math.acos(2. *math.pi* (v ) )
    ...
    for mol in range(Ns):
        for j in range(mol+1,Ns):
            ...
            for trans in pos_trans:
                ...
                uij = vel[mol] - vel[j]
                am = sum(uij**2)
                # am = la.multiply(uij,uij)
                # am = np.linalg.dot(uij,uij)
                # bm = np.linalg.dot(rij,uij)
                bm = np.sum(rij*uij)
