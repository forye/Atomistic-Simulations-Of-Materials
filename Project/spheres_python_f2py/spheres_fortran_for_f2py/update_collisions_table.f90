...
!  till next collision for each pair of spheres.
    ...
    integer :: i
    double precision, parameter :: infinity = 9999999.0!huge(0.0d0)
    integer :: j
    integer :: jx
    integer :: jy
    integer :: jz
                ...
                do jx = -1, 1
                    do jy = -1, 1
                        do jz = -1, 1
                            ...
                            if (bij < 0.0) then
                                cij = dot_product(rij,rij) - sigma_sq
                                aij = dot_product(uij,uij)
                                discriminant = bij * bij - aij * cij
                                if (discriminant > 0.0) then
                                    time = - ( bij + sqrt(discriminant)) / aij
